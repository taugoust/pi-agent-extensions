...
import type { TuiWorkerJobParams } from "./tui-worker-protocol.ts";

/** Trusted in-process controller, permanently bound to the actual local Pi
...
export type JobParams = {
  action: "start" | "adopt" | "list" | "status" | "output" | "wait" | "signal" | "cancel" | "reap" | "watch" | "watches" | "events" | "ack" | "unwatch";
...
// Trusted controller metadata, never accepted as model-supplied JobParams.
export type JobPlacement = { socketPath: string; sessionId: string; windowId: string; paneId: string; panePid: number; paneStartToken: string };
export type ParentJobIdentity = { sessionId: string; childId: string; cwd: string; placement?: JobPlacement };
...
export const JobParameters = Type.Object({
  action: Type.String({ pattern: "^(start|adopt|list|status|output|wait|signal|cancel|reap|watch|watches|events|ack|unwatch)$" }),
...
export function validateJobParams(p: JobParams): void {
  ...
  const allowed: Record<string,string[]> = {
    ...
    output:["job_id","lines"], wait:["job_id","timeout_ms","lines"], signal:["job_id","signal"], cancel:["job_id"], reap:["job_id"],
  };
