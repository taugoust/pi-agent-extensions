...
For the complete draw.io XML reference including common styles, edge routing, containers, layers, tags, metadata, dark mode colors, and XML well-formedness rules, read and follow `references/xml-reference.md` from this skill directory.

## Existing diagrams / prior work

When the user asks to base a diagram on existing diagrams from prior work, read the relevant `.drawio`, `.drawio.svg`, or `.drawio.png` files first. Extract the structure, terminology, visual conventions, and reusable mxGraph style strings, then generate a new `.drawio` file rather than modifying prior-work originals unless explicitly asked.
