...
#+html:<details>
#+html:<summary><strong>modal-editor</strong> - Vim-style modal input editor</summary>
#+html:<br>

- *Source*: [[https://github.com/rytswd/pi-agent-extensions/tree/main/modal-editor][modal-editor/]]
- *License*: MIT
- *Origin*: Based on [[https://github.com/badlogic/pi-mono/tree/main/packages/coding-agent/examples/extensions/modal-editor.ts][badlogic/pi-mono]]

*Description*: Replaces the default pi input editor with a vim-style modal editor. Adds an =INSERT= / =NORMAL= mode indicator to the bottom border of the editor.
...
| =a=      | normal  | Append (insert + move right)|
| =h=      | normal  | Move left                   |
| =j=      | normal  | Move down                   |
| =k=      | normal  | Move up                     |
| =l=      | normal  | Move right                  |
| =0=      | normal  | Jump to line start          |
| =$=      | normal  | Jump to line end            |
| =x=      | normal  | Delete character            |
| =Escape= | normal  | Abort agent (default pi)    |
...
- *Toggle*: =--no-sandbox= flag to disable; configure =enabled: false= in config; =/sandbox-control= to toggle within a session
- *Command*: =/sandbox= to show current configuration; =/sandbox-control= to enable/disable
