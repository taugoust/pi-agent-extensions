...
*Description*: Intercepts =write= and =edit= tool calls and blocks any that target a path outside the current working directory. Complements the =sandbox= extension, which restricts bash at the OS level, by closing the same gap for pi's native file tools.

*How it works*:
- Resolves the target path (absolute or relative) against =ctx.cwd=
- Normalises both paths to prevent =..= traversal
- Returns a clear block reason to the LLM when a path is outside =cwd=
- No-op for all other tool calls (e.g., bash)

*What it looks like* when a write is blocked:

#+begin_example
[tool blocked] Path is outside the working directory (/home/user/my-project):
/etc/passwd. Only paths inside /home/user/my-project are allowed while
fence is active.
#+end_example
