...
Root list/tasks merge native and retained legacy inventory. Global `wait_any`/`wait_all` use one active snapshot across both backends and one deadline; cancellation stops observation only. Group-specific controls remain backend-owned. Structured routine updates go through `quietState.enqueue`, with persisted per-child replay cursors; there are no routine parent transcript injections. Background terminal execution snapshots (including failed/lost launches) enqueue deduplicated completion wake-ups with job/child result-routing IDs. They wake idle parents or steer at the next tool-safe boundary, without sharing the guidance quota/throttle. Foreground results and replayed historical settlements do not wake again; task outcomes alone remain quiet until settlement. Explicit guidance is separately bounded. Findings are rate-limited and remain retained alongside model-reported task outcomes.
