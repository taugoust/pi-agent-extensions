...
test("child-local job control cannot start shells or override ownership scope", () => {
  ...
  for (const params of [{ action: "list", limit: 50 }, { action: "output", job_id, lines: 2000 }, { action: "wait", job_id, timeout_ms: 43_200_000 },
  ...
  for (const params of [{ action: "start", command: "false" }, { action: "adopt", pane_id: "%1" }, { action: "signal", job_id, signal: "SIGTERM" },
    ...
    { action: "wait", job_id, timeout_ms: 43_200_001 }, { action: "output", job_id: "../foreign" }, { action: "ack", watch_id }]) assert.throws(() => parseTuiWorkerJobParams(params));
});
