...
export function validateOutcomeSummaries(value:any):TaskOutcomeSummary[] {
 ...
 return value.map(v=>{if(!v||!Number.isInteger(v.child)||v.child<1||v.child>8||!['delivered','partial','blocked','checkpointed','unreported'].includes(v.state)||typeof v.reported!=='boolean')throw new Error('Invalid task outcome summary');str(v.summary,4096,'summary');if(v.child_id!==undefined&&!/^subagent-child-[0-9a-f]{24}$/.test(v.child_id))throw new Error('Invalid outcome child identity');if(v.next_action!==undefined)str(v.next_action,4096,'next action');if(v.reported!==(v.state!=='unreported'))throw new Error('Invalid reported outcome flag');if(v.task_id!==undefined&&!/^subagent-task-[0-9a-f]{24}$/.test(v.task_id))throw new Error('Invalid outcome task identity');if(v.attempt!==undefined&&(!Number.isSafeInteger(v.attempt)||v.attempt<1))throw new Error('Invalid task attempt');return {child:v.child,child_id:v.child_id,task_id:v.task_id,attempt:v.attempt,state:v.state,reported:v.reported,summary:v.summary,next_action:v.next_action};});
}
